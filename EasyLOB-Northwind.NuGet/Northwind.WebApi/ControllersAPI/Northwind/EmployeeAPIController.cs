...
namespace Northwind.WebApi
{
    ...
    public class EmployeeAPIController : BaseApiControllerApplicationDTO<EmployeeDTO, Employee>
    {
        #region Methods
        ...
        [Route("{employeeId}")]
        public IHttpActionResult DeleteEmployee([FromUri] int employeeId)
        {
            ...
            try
            {
                ...
                {
                    object[] ids = new object[] { employeeId };
                    EmployeeDTO employeeDTO = Application.GetById(operationResult, ids);
        ...
        [Route("{employeeId}")]
        public IHttpActionResult GetEmployee([FromUri] int employeeId)
        {
            ZOperationResult operationResult = new ZOperationResult();
        ...
        public IHttpActionResult GetEmployees([FromUri] string where = null,
            ...
            [FromUri] int? take = null)
        {
            ...
            try
            {
                if (IsSearch(operationResult))
                {
                    ...
                    IEnumerable<EmployeeDTO> result = Application.Search(operationResult,
        ...
        public IHttpActionResult PostEmployee([FromBody] EmployeeDTO employeeDTO)
        {
            ...
            try
            {
                if (IsCreate(operationResult))
                {
                    ...
                    {
                        return Ok(employeeDTO.ToData().GetId());
                    }
                }
            }
            catch (Exception exception)
        ...
        public IHttpActionResult PutEmployee([FromBody] EmployeeDTO employeeDTO)
        {
            ...
            try
            {
                ...
                {
                    object[] ids = employeeDTO.ToData().GetId();
                    EmployeeDTO dto = Application.GetById(operationResult, ids);
                    if (operationResult.Ok)
                    {
                        if (dto != null)
                        {
                            if (Application.Update(operationResult, employeeDTO))
                            {
                                return Ok(ids);
                            }
                        }
