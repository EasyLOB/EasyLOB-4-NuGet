...
function zAlert(message, modal = true) {
    ...
    // https://ej2.syncfusion.com/javascript/documentation/dialog/es5-getting-started/
    // In the dialog control, max-height is calculated based on the dialog target element height.
    // If the target property is not configured, the document.body is considered as a target.
    // Therefore, to show a dialog in proper height, you need to add min-height to the target element.
...
function zReadOnly(parentId, readOnly = true) {
    ...
    if (readOnly) {
        ...
        $(parentId + " input.e-datepicker").each(function () {
            this.ej2_instances[0].readonly = true;
        });
        $(parentId + " input.e-datetimepicker").each(function () {
            this.ej2_instances[0].readonly = true;
        });
